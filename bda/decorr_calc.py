...
def _dudt(lx, ly, hour_angle, earth_omega, wavelength):
    ...
    Dosctring goes here
    """
    hour_angle = max(min(hour_angle, Angle(90., units.degree)), Angle(-90., units.degree))
    return (lx * np.cos(hour_angle) - ly * np.sin(hour_angle)) * earth_omega / wavelength
...
def _dvdt(lx, ly, hour_angle, dec, earth_omega, wavelength):
    ...
    return (lx * np.sin(dec) * np.sin(hour_angle)
            + ly * np.sin(dec) * np.cos(hour_angle)) * earth_omega / wavelength
...
def decorr_pre_fs_int_time(frequency, baseline, pre_fs_int_time):
    ...
    return float(pre_fs_int_time * earth_rot_speed
...
def decorr_chan_width(chan_width, baseline, corr_FoV):
    ...
    Docstring goes here
    """
    return float(chan_width.to(1 / units.s) * baseline
...
def decorr_post_fs_int_time(lx, ly, post_fs_int_time, corr_FoV, frequency,
    ...
    # case 3: +m
    dv = _dvdt(lx, ly, 0., telescope_latitude + corr_FoV, earth_rot_speed, wavelength)
    ...
    int_time = np.pi**2 * (post_fs_int_time.to(units.s).value)**2 / 6. * rfac.value
